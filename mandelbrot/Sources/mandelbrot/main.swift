...
typealias complex = Complex<float>


func mandel_single_thread(h : Int, w: Int, center : complex, range : complex, iters : Int) -> [UInt32] {
	let FIRE_PALETTE : [UInt32] = [0, 0, 4, 12, 16, 24, 32, 36, 44, 48, 56, 64, 68, 76, 80, 88, 96, 100, 108, 116, 120, 128, 132,
						...
						72, 68, 64, 60, 56, 52, 48, 44, 40, 36, 32, 28, 24, 20, 16, 12, 8, 0, 0].map({$0 | 0xff00_0000})


	
	func do_scale(cr : complex, range : complex, i  : Int, j : Int, w : Int, h : Int) -> complex {
        return cr + complex((range.imaginary - range.real) * float(i) / float(w), (range.imaginary - range.real) * float(j) / float(h))
	}

	var image = Array(repeating: UInt32(0), count: w * h)

	let scale = 0.8 *  float(w) / float(h)
	let cr = complex(range.real, range.real)
	...
	for index in 0..<h*w {
			...
			let c0 = do_scale(cr : cr, range : range, i : i, j : j, w : w, h : h) * complex(scale) - center
			var z = c0
			...
			for i in 0...iters {
				...
				if z.lengthSquared > 4.0 { ix=i;  break }
			}
			image[index] =  (ix >= iters) ? UInt32(0xff00_0000) : FIRE_PALETTE[(FIRE_PALETTE.count * ix / 50) % FIRE_PALETTE.count] 
...
func mandel_parallel(h : Int, w: Int, center : complex, range : complex, iters : Int) -> [UInt32] {
	let FIRE_PALETTE : [UInt32] = [0, 0, 4, 12, 16, 24, 32, 36, 44, 48, 56, 64, 68, 76, 80, 88, 96, 100, 108, 116, 120, 128, 132,
						140, 148, 152, 160, 164, 172, 180, 184, 192, 200, 1224, 3272, 4300, 6348, 7376, 9424, 10448,
						12500, 14548, 15576, 17624, 18648, 20700, 21724, 23776, 25824, 26848, 28900, 29924, 31976,
						33000, 35048, 36076, 38124, 40176, 41200, 43248, 44276, 46324, 47352, 49400, 51452, 313596,
...
func main() {
	let mf = 8, w = mf*1024, h = mf*1024
	print("running mandel on \(w) x \(h) = \(w*h)...")

	var image = [UInt32]()

	for _ in 0..<3 {
		let clock = Clock.system, now =  clock.thisInstant() 	// retrieve the current calendar day, as defined by the user's region

		image = mandel_parallel(h : 1024*mf, w : 1024*mf, center : complex(0.5, 0), range : complex(-2,+2), iters: 200)
		
		print("lap=\((clock.thisInstant()-now))")
	}
	
	let data = Data(bytes: image, count: image.count * MemoryLayout<UInt32>.size) // write data	
	try? data.write(to: URL(fileURLWithPath: "mandel.bin"), options: .atomic)
